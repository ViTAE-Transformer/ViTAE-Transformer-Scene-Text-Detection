...
def imshow_det_bboxes(img,
    ...
    for i, (bbox, label) in enumerate(zip(bboxes, labels)):
        ...
        if segms is not None:
            ...
            poly_res = res_list[-2]
            poly_res = [x.flatten() for x in poly_res]
            poly_res = [x for x in poly_res if len(x) >= 6]
            if len(poly_res) > 0:
                demo_polygon_list.append(poly_res[0])
                demo_score_list.append(demo_score_list_pre[i])
